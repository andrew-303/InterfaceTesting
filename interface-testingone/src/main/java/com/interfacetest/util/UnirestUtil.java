...
public class UnirestUtil {
	...
	public static String InterfaceTest(Map<String,String> data){
		...
		String uri = data.get("interface_url");
		...
		try {
			...
			if(expect.toLowerCase().contains("select")){
					...
					try {
						while(qrs1.next()){
							...
							if(num.equals("1")){
								...
								Assert.assertEquals(responseBody,expect);
							}
						}
				...
				if(responseBody.contains(expect) && !PostInterfaceTestUtil.extractResult(responseBody) ){
					flag = true;
					logger.info("接口测试，post请求正常响应，数据正确返回");
					logger.info("ResponseBody:" + responseBody);
					Assert.assertTrue(flag);					
